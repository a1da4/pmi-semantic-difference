...
def main(args):
    ...
    if args.similarity_matrices is not None:
        print("1. load sims_mat ...", end="")
        sims_path_base, sims_path_other = args.similarity_matrices
        sims_mat_base = np.load(sims_path_base)
        sims_mat_other = np.load(sims_path_other)
        print("done")
    else:
        print("1. create sims_mat ...", end="")
        assert args.word_matrices is not None, "MATRICES NOT FOUND ERROR"
        c_path_base, c_path_other = args.word_matrices
...
def cli_main():
    ...
    parser.add_argument(
        "-m", "--word_matrices", nargs=2, help="path of cooccur/wordvector matrices"
    )
    parser.add_argument(
        "-s", "--similarity_matrices", nargs=2, help="path of similarity matrices"
    )
